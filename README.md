...
- At startup installs build tools (`gcc`, `libpcre2-dev`) plus `transformers[serving]`, `accelerate`, `kernels`, `triton`, `openai-harmony`, and **`gptqmodel`** (required to load AWQ models; `pypcre` compiles from source).
